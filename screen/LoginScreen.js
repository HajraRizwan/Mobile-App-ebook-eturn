...
const LoginScreen = ({ navigation }) => {//making component over here
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      await signInWithEmailAndPassword(auth, email, password);//function of firebase
      // Navigate to the Home screen if successful
      navigation.navigate('Home');
    } catch (error) {//error.code is a unique string from firebase that describe type of error
      // Handle specific errors
      if (error.code === 'auth/invalid-email') {//check format of email
        Alert.alert('Error', 'The email address is invalid.');
      } else if (error.code === 'auth/user-not-found') {//user not available on firebase
        Alert.alert('Error', 'No user found with this email.');
      } else if (error.code === 'auth/wrong-password') {//wrong password
  ...
  return (
    <View style={styles.container}>
      {/* Wave Animation */}
      <Svg height="40%" width="100%" viewBox="0 0 1440 320" style={styles.wave}>//drawing area size and position 
        <Path
          fill="#fbedf2"
